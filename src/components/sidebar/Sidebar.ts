...
class SidebarItem extends ListItemElement {
	...
	items(r: {
		...
		for (let i = 0; i < r.length; i++) {
			const element = new ListItemElement({ children: [new AnchorElement({ marginLeft: "0.5rem", href: "#", text: r[i].title, className: "text-link" })] });
			element.onclick = () => {
...
export class Sidebar extends Component {
	constructor() {
		...
		for (let i = 0; i < data.sidebar.length; i++) {
			order.append(new SidebarItem({ text: data.sidebar[i].title, items: data.sidebar[i].items }));
		}
	}
}
